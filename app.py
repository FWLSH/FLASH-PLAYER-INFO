...
async def get_region_by_uid(uid: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.get(f"https://ff.garena.com/api/antihack/check?uid={uid}")
...
def get_account_info():
    ...
    try:
        ...
        region = loop.run_until_complete(get_region_by_uid(uid))
        ...
        return jsonify({"error": f"Invalid UID or server error: {str(e)}"}), 500
...
def startup_sync():
    ...
    asyncio.set_event_loop(loop)
    loop.run_until_complete(initialize_tokens())
    asyncio.ensure_future(refresh_tokens_periodically(), loop=loop)
    return loop

if __name__ == '__main__':
    # Start background tasks
    background_loop = startup_sync()
    
    # Run Flask app
    app.run(host='0.0.0.0', port=5080, debug=True, threaded=True)import asyncio
import time
import httpx
import json
from collections import defaultdict
